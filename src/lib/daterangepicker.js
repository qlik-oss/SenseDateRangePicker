...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root.qlikdaterangepicker = factory(
      root, {}, root.moment || momentjs, (root.jQuery || root.Zepto || root.ender || root.$));
  }
  ...
  var DateRangePicker = function (element, options, cb) {
    ...
    if (typeof options.template !== 'string')
      options.template = `
        <div id="dropDown_${options.id}" div class="qlik-daterangepicker dropdown-menu" style="display:none">
  ...
  DateRangePicker.prototype = {
    ...
    renderCalendar: function (side) {
      ...
      if ((!this.linkedCalendars || side == 'left')) {
        ...
        html += `"><button type="button" class="btn btn-default btn-xs" aria-label="Left Align">
          <i class="lui-icon lui-icon--previous lui-icon--small"></i></button></th>`;
      } else {
        html += '<th></th>';
      }
      ...
      if ((!this.linkedCalendars || side == 'right' || this.singleDatePicker)) {
        ...
        html += `"><button type="button" class="btn btn-default btn-xs" aria-label="Left Align">
          <i class="lui-icon lui-icon--next lui-icon--small"></i></button></th>`;
      } else {
        html += '<th></th>';
      }
    ...
    updateFormInputs: function () {
      ...
      if (this.container.find('input[name=qlik-daterangepicker_start]').is(":focus")
        || this.container.find('input[name=qlik-daterangepicker_end]').is(":focus")) {
        return;
      }
      ...
      if (this.endDate && this.endDate._isValid) { //eslint-disable-line
        this.container.find('input[name=qlik-daterangepicker_end]')
    ...
    show: function (e) {
      ...
      this._outsideClickProxy = $.proxy(function (e) { this.outsideClick(e); }, this); //eslint-disable-line

      // Bind global datepicker mousedown for hiding and
      $(document)
        .on('mousedown.qlik-daterangepicker', this._outsideClickProxy) //eslint-disable-line
        // also support mobile devices
        .on('touchend.qlik-daterangepicker', this._outsideClickProxy) //eslint-disable-line
        // also explicitly play nice with Bootstrap dropdowns, which stopPropagation when clicking them
        .on('click.qlik-daterangepicker', '[data-toggle=dropdown]', this._outsideClickProxy) //eslint-disable-line
        // and also close when focus changes to outside the picker (eg. tabbing between controls)
        .on('focusin.qlik-daterangepicker', this._outsideClickProxy); //eslint-disable-line
    ...
    clickDate: function (e) {
      ...
      if (this.endDate || date.isBefore(this.startDate)) {
        if (this.timePicker) {
          let hour = parseInt(this.container.find('.dpleft .hourselect').val(), 10);
          if (!this.timePicker24Hour) {
          ...
          let minute = parseInt(this.container.find('.dpleft .minuteselect').val(), 10);
          let second = this.timePickerSeconds ? parseInt(this.container.find('.dpleft .secondselect').val(), 10) : 0;
        ...
        if (this.timePicker) {
          let hour = parseInt(this.container.find('.dpright .hourselect').val(), 10);
          if (!this.timePicker24Hour) {
            let ampm = this.container.find('.dpright .ampmselect').val();
            if (ampm === 'PM' && hour < 12)
              hour += 12;
            if (ampm === 'AM' && hour === 12)
              hour = 0;
          }
          let minute = parseInt(this.container.find('.dpright .minuteselect').val(), 10);
          let second = this.timePickerSeconds ? parseInt(this.container.find('.dpright .secondselect').val(), 10) : 0;
          date = date.clone().hour(hour).minute(minute).second(second);
        }
